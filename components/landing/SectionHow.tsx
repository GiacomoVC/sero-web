...
export function SectionHow() {
  return (
    <section className="bg-ink py-24 sm:py-32 text-white overflow-hidden">
      {/* Heading */}
      ...
      {/* Swipe carousel — snaps on mobile, 3-col grid on sm+ */}
      <div className="sm:hidden -mx-0 px-6 overflow-x-auto snap-x snap-mandatory scroll-smooth flex gap-4 pb-4">
        {STEPS.map((step) => (
          <div
            key={step.num}
          ...
          >
            <span className={`text-5xl font-black block ${step.highlighted ? 'text-white/30' : 'text-coral/30'}`}>
              {step.num}
            </span>
            <span className="text-3xl block mt-4">{step.icon}</span>
            <h3 className="text-xl font-bold mt-3">{step.title}</h3>
            <p className={`text-sm mt-2 leading-relaxed ${step.highlighted ? 'text-white/80' : 'text-white/50'}`}>
              {step.body}
            </p>
          </div>
        ))}
      </div>
      <p className="sm:hidden mt-3 text-center text-white/25 text-xs tracking-wide">
        desliza →
      </p>
